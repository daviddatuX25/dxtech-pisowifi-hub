-- 0006_branch_isolated_vouchers.sql
-- Enforces branch isolation for voucher pools:
-- 1. Backfill assigned legacy vouchers with the profile/request branch.
-- 2. Transition unique constraint from (promotion_id, code) to (promotion_id, branch_id, code).
-- 3. Update claim_voucher_promo RPC to strictly draw vouchers matching the claimant's branch.

-- Backfill claimed vouchers that had null branch_id using request and profile branch
update public.promotion_vouchers pv
set branch_id = pr.branch_id
from public.promo_requests pr
where pv.assigned_request_id = pr.id
  and pv.branch_id is null;

update public.promotion_vouchers pv
set branch_id = p.branch_id
from public.profiles p
where pv.assigned_profile_id = p.id
  and pv.branch_id is null;
...
begin
  if exists (
    ...
    alter table public.promotion_vouchers
      drop constraint promotion_vouchers_unique_code;
  end if;
end;
$$;

-- Add composite unique constraint per branch
do $$
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.promotion_vouchers'::regclass
      and conname = 'promotion_vouchers_unique_branch_code'
  ) then
    alter table public.promotion_vouchers
      add constraint promotion_vouchers_unique_branch_code unique (promotion_id, branch_id, code);
  end if;
end;
$$;

-- Optimize index for branch-specific unassigned voucher lookups
create index if not exists promotion_vouchers_branch_available_idx
  on public.promotion_vouchers(promotion_id, branch_id, created_at)
  where assigned_profile_id is null;

-- Update claim_voucher_promo to strictly require branch_id = p_branch_id
...
begin
  ...
  -- Strictly draw from the claiming customer's branch voucher pool
  select *
